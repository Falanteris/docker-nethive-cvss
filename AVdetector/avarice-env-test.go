...
func main(){
	...
	//Env Variable Check..
	iface_string := os.Getenv("INTERFACE");
	iface_pair := strings.Split(iface_string,"/");
	if len(iface_pair) != 2{
		panic("Argument should be IP/CIDR format (example 100.100.100.100/24");
	}
	subnets,addr_loc,_ := GenerateSubnet(iface_pair[0],iface_pair[1])
	localip,err := ParseIp(iface_pair[0])
